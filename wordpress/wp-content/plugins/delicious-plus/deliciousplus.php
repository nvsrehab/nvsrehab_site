...
function widget_deliciousplus($args, $widget_args = 1) {
	...
	$json_url = 'http://feeds.delicious.com/v2/json/tag/' . rawurlencode($options['username']);
	$json_url.= $tags ? '/' . rawurlencode(implode('+', $tags)) : '';
	$json_url.= '?count=' . ((int) $options['count']) . '&callback=makeItDelicious';
	
	echo $before_widget;
	echo $before_title . "<a href='http://delicious.com/tag/{$options['username']}'>{$options['title']}</a>" . $after_title;
	?>
